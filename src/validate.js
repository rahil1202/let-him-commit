...
export const userInputSchema = z.object({
  ...
  batchSize: z.coerce.number()
    .min(1)
    .max(500, "Batch size must be ≤ 500."),

  commitTemplate: z.string().min(1, "Commit message template required.")
})

.refine(
  (data) => data.maxCommitsPerDay >= data.minCommitsPerDay,
  { message: "Max commits per day must be ≥ Min commits per day.", path: ["maxCommitsPerDay"] }
)
.refine((data) => {
