...
export async function checkRateLimit(token: string): Promise<void> {
  ...
  else logInfo(`API Rate limit OK (${remaining} calls remaining)`);
}

export async function ensureRepo(token: string, username: string, repoName: string): Promise<void> {
  const headers = { Authorization: `token ${token}` };
  ...
  try {
    ...
    await retry(() =>
      axios.post("https://api.github.com/user/repos", { name: repoName, private: true }, { headers })
    );
